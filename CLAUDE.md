...
1. **Zero trust.** Every request's Keycloak JWT is validated locally (JWKS, `iss`, `exp`, optional `aud`) in `auth.py`'s ASGI middleware *before* it reaches the MCP transport. The validated `UserContext` (including the raw token) travels via a contextvar (`ump_mcp.auth.current_user()`). When `resource_url` is set the server also acts as an OAuth resource server: `app.py:_discovery_routes` publishes RFC 9728 metadata (unauthenticated, at both the path-inserted and bare `/.well-known/oauth-protected-resource`) and the middleware's RFC 6750 challenge names it, so clients run auth-code + PKCE themselves. Keycloak is the authorization server — this service never issues or refreshes tokens.
...
- **The landing page is the only human-facing surface.** `landing.py` renders `GET /` (EN/DE from `Accept-Language`, `?lang=` overrides) and serves its fonts and the brand mark from `src/ump_mcp/static/` — no CDN, no build step. Its tokens are copied from the Claude Design project *Urban Futures Collective Design System* (`tokens/*.css`: Poppins headings/UI + Satoshi body, yellow as the only accent, square corners, Utopia clamp scales); re-sync with `/design-sync` rather than inventing values. Note this is the UFC *website* system, which differs from `../ump-x-frontend`'s Nuxt tokens — don't mix the two. `/mcp` stays a machine surface: a browser GET there must keep returning the 401 challenge, never HTML.
